...
class Formula:
    ...
    def backtracking(self, estado=None):
        if estado is None:
            estado = nodos_tableaux([self])
        res = estado.es_hoja()
        if res == 'cerrada':
...
def dpll(S, I):
    ...
    l = choice([literal for clause in S for literal in clause if literal not in I and complemento(literal) not in I])
    ...
    if r == 'Satisfacible':
        ...
        S2 = eliminar_literal(S, complemento(l))
        I2 = extender_I(I, complemento(l))
        return dpll(S2, I2)
...
def walkSAT(S, max_flips, max_tries, p):
    for i in range(max_tries):
        w = WalkSatEstado(S)
        for j in range(max_flips):
            if w.SAT():
                return w.I
            C = w.clausulas_unsat[randint(0, len(w.clausulas_unsat)-1)]
            if any((w.break_count(x)==0 for x in C)):
                l = choice(C)
            else:
                l = min(C, key=w.break_count)
            if uniform(0,1) < p:
                l = choice(C)
            w.actualizar(flip_literal(w.I, l))
    return None
